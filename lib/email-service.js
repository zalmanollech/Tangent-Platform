...
async function initEmailService() {
    try {
        if (EMAIL_PROVIDER === 'nodemailer') {
            ...
            if (!RESEND_API_KEY) {
                console.warn('[WARN] Resend API key not configured. Email service disabled.');
                return null;
            }
            
            resendClient = new Resend(RESEND_API_KEY);
            console.log('[INFO] Email service (Resend) initialized successfully');
            return resendClient;
...
async function sendEmailWithResend(to, subject, html, text) {
    try {
        ...
        if (!resendClient) {
            console.warn('[WARN] Resend client not available. Skipping email send.');
            return false;
        }
...
async function sendEmail(to, subject, html, text) {
    try {
        if (EMAIL_PROVIDER === 'nodemailer') {
            ...
            console.warn('[WARN] Email provider not implemented:', EMAIL_PROVIDER);
            return false;
        }
    } catch (error) {
        console.error('[ERROR] Email send error:', error.message);
