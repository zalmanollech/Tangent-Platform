...
<body>
    ...
    ${report.dataSources || report.companyIdentification ? `
    <div class="section" style="background: #fff7ed; border-left: 4px solid #f59e0b;">
        <div class="section-title">📊 Data Sources & Reliability</div>
        <div style="padding: 20px; background: white; border-radius: 8px; margin-top: 15px;">
            ${report.companyIdentification ? `
            <div style="margin-bottom: 20px; padding-bottom: 15px; border-bottom: 1px solid #ddd;">
                <strong style="color: #f59e0b;">Company Identification:</strong>
                <div style="margin-top: 10px;">
                    <div style="margin-bottom: 8px;">
                        <strong>Level:</strong> ${report.companyIdentification.level}
                        ${report.companyIdentification.level === 'WEAK' || report.companyIdentification.level === 'VERY_WEAK' ? ' ⚠️' : ''}
                    </div>
                    <div style="margin-bottom: 8px;">
                        <strong>Provided:</strong> ${report.companyIdentification.provided.length > 0 ? report.companyIdentification.provided.join(', ') : 'None'}
                    </div>
                    ${report.companyIdentification.missing && report.companyIdentification.missing.length > 0 ? `
                    <div style="margin-bottom: 8px; color: #dc2626;">
                        <strong>Missing:</strong> ${report.companyIdentification.missing.join(', ')}
                    </div>
                    ` : ''}
                    <div style="margin-bottom: 8px;">
                        <strong>Confidence:</strong> ${(report.companyIdentification.confidence * 100).toFixed(0)}%
                    </div>
                </div>
            </div>
            ` : ''}
            ...
            <div style="margin-bottom: 20px;">
                <strong style="color: #f59e0b;">Data Reliability:</strong>
                <div style="margin-top: 10px;">
                    <div style="margin-bottom: 8px;">
                        <strong>Overall Reliability:</strong> ${report.dataSources.overallReliability}
                        ${report.dataSources.overallReliability === 'VERY_LOW' || report.dataSources.overallReliability === 'LOW' ? ' ⚠️' : ''}
                    </div>
                    <div style="margin-bottom: 8px;">
                        <strong>Reliability Score:</strong> ${report.dataSources.reliabilityScore}%
                    </div>
                    <div style="margin-bottom: 8px;">
                        <strong>Confidence:</strong> ${(report.dataSources.confidence * 100).toFixed(0)}%
                    </div>
                </div>
            </div>
            
            <div style="margin-bottom: 20px;">
                <strong style="color: #f59e0b;">Credit Bureaus Checked:</strong>
                <div style="margin-top: 10px;">
                    ${report.dataSources.checked && report.dataSources.checked.length > 0 && !report.dataSources.checked[0].includes('None') ? `
                    <div style="margin-bottom: 8px; color: #10b981;">
                        ✓ ${report.dataSources.checked.join(', ')}
                    </div>
                    ` : `
                    <div style="margin-bottom: 8px; color: #dc2626;">
                        ⚠️ None - Algorithmic estimate only
                    </div>
                    `}
                    ${report.dataSources.notChecked && report.dataSources.notChecked.length > 0 ? `
                    <div style="margin-bottom: 8px; color: #dc2626;">
                        ⚠️ Not checked: ${report.dataSources.notChecked.join(', ')}
                    </div>
                    ` : ''}
                </div>
            </div>
            ...
            <div style="margin-top: 20px; padding-top: 15px; border-top: 1px solid #ddd;">
                ...
                <ul style="margin-top: 10px; padding-left: 20px;">
                    ...
                    <li style="margin-bottom: 8px; color: ${warning.severity === 'HIGH' ? '#dc2626' : '#f59e0b'};">
                        ${warning.message}
                        ${warning.recommendation ? `<br><small style="color: #666;">Recommendation: ${warning.recommendation}</small>` : ''}
                    </li>
                    `).join('')}
                </ul>
            </div>
