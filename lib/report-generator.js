...
async function generateMockCreditReport(formData) {
    ...
    const countryAdjustment = countryRiskFactors[country] || countryRiskFactors['DEFAULT'];
    score += countryAdjustment;
    scoreBreakdown.countryAdjustment = countryAdjustment;
    ...
    const sectorAdjustment = sectorRiskFactors[matchedSector];
    score += sectorAdjustment;
    scoreBreakdown.sectorAdjustment = sectorAdjustment;
    
    // Trade Value Risk (non-linear scaling)
    let tradeValueAdjustment = 0;
    if (tradeValue > 5000000) tradeValueAdjustment = -15; // Very large trades
    else if (tradeValue > 1000000) tradeValueAdjustment = -10; // Large trades
    else if (tradeValue > 500000) tradeValueAdjustment = -5; // Medium-large trades
    else if (tradeValue < 50000) tradeValueAdjustment = 3; // Small trades (lower risk)
    score += tradeValueAdjustment;
    scoreBreakdown.tradeValueAdjustment = tradeValueAdjustment;
    
    // Tenor Risk (payment terms)
    let tenorAdjustment = 0;
    if (tenor > 180) tenorAdjustment = -20; // Very long terms
    else if (tenor > 120) tenorAdjustment = -15; // Long terms
    else if (tenor > 90) tenorAdjustment = -8; // Medium-long terms
    else if (tenor < 30) tenorAdjustment = 5; // Short terms (lower risk)
    score += tenorAdjustment;
    scoreBreakdown.tenorAdjustment = tenorAdjustment;
    
    // Role-based adjustments
    let roleAdjustment = 0;
    if (role === 'supplier') roleAdjustment = 2; // Suppliers typically have lower risk
    else if (role === 'buyer') roleAdjustment = -1; // Buyers have slightly higher risk
    score += roleAdjustment;
    scoreBreakdown.roleAdjustment = roleAdjustment;
    ...
    // Enhanced factors with more realistic data (initial)
    const factors = {
        pd: pd,
        risk_band: riskBand,
    ...
    // Calculate confidence levels for each component (now that we have data source reliability)
    const confidenceLevels = {
        baseScore: 1.0, // Always 100% confident in base score
        country: identificationStrength.level === 'STRONG' ? 0.95 : identificationStrength.level === 'MEDIUM' ? 0.85 : 0.70,
        sector: identificationStrength.level === 'STRONG' ? 0.90 : identificationStrength.level === 'MEDIUM' ? 0.80 : 0.65,
        tradeValue: 0.95, // High confidence in numeric values
        tenor: 0.95, // High confidence in numeric values
        role: 0.90, // Good confidence in role
        overall: dataSourceReliability ? dataSourceReliability.overallReliability : 0.75 // Overall confidence
    };
    
    // Add confidence levels to factors
    factors.confidenceLevels = confidenceLevels;
    
    // Track what data sources were actually checked and verified
    // Safely check credit bureau data structure
    const creditBureauData = mockCreditBureauScores && mockCreditBureauScores.scores ? mockCreditBureauScores.scores : {};
    
    const dataSourcesChecked = {
        companyRegistry: !!registrationNumber, // Checked if registration number provided
        creditBureaus: {
            dnb: !!(creditBureauData.dnb && creditBureauData.dnb.score !== null && creditBureauData.dnb.score !== undefined),
            experian: !!(creditBureauData.experian && creditBureauData.experian.score !== null && creditBureauData.experian.score !== undefined),
            equifax: !!(creditBureauData.equifax && creditBureauData.equifax.score !== null && creditBureauData.equifax.score !== undefined)
        },
        kycVerification: false, // Would be true if KYC documents were available
        sanctionsCheck: false, // Would be true if real sanctions API was available
        financialAnalysis: !!(mockCreditBureauReport && mockCreditBureauReport.financialAnalysis),
        paymentHistory: !!(mockCreditBureauReport && mockCreditBureauReport.paymentHistory),
        bankruptcyCheck: !!(mockCreditBureauReport && mockCreditBureauReport.bankruptcyHistory)
    };
    
    // What verification steps were performed
    const verificationSteps = {
        companyIdentification: identificationStrength.level === 'STRONG' ? 'VERIFIED' : 
                               identificationStrength.level === 'MEDIUM' ? 'PARTIAL' : 'WEAK',
        companyRegistryLookup: !!registrationNumber ? 'ATTEMPTED' : 'NOT_ATTEMPTED',
        creditBureauQueries: Object.values(dataSourcesChecked.creditBureaus).some(v => v) ? 'ATTEMPTED' : 'NOT_ATTEMPTED',
        sanctionsScreening: 'SIMULATED', // Would be 'VERIFIED' with real API
        financialAnalysis: dataSourcesChecked.financialAnalysis ? 'ATTEMPTED' : 'NOT_ATTEMPTED',
        paymentHistoryCheck: dataSourcesChecked.paymentHistory ? 'ATTEMPTED' : 'NOT_ATTEMPTED',
        bankruptcyCheck: dataSourcesChecked.bankruptcyCheck ? 'ATTEMPTED' : 'NOT_ATTEMPTED'
    };
    
    // Add data sources and verification info to factors
    factors.dataSourcesChecked = dataSourcesChecked;
    factors.verificationSteps = verificationSteps;
    ...
    // Add credit bureau data to factors (with null checks)
    factors.credit_bureau_score = mockCreditBureauScores && mockCreditBureauScores.averageScore !== undefined ? mockCreditBureauScores.averageScore : null;
    factors.credit_bureau_rating = mockCreditBureauScores && mockCreditBureauScores.overallRating ? mockCreditBureauScores.overallRating : 'UNKNOWN';
    factors.credit_bureau_reliability = mockCreditBureauScores && mockCreditBureauScores.reliability ? mockCreditBureauScores.reliability : 0;
    factors.credit_bureau_sources = mockCreditBureauScores && mockCreditBureauScores.sources ? mockCreditBureauScores.sources : {};
    factors.financial_analysis = mockCreditBureauReport && mockCreditBureauReport.financialAnalysis ? mockCreditBureauReport.financialAnalysis : null;
    factors.payment_history = mockCreditBureauReport && mockCreditBureauReport.paymentHistory ? mockCreditBureauReport.paymentHistory : null;
    factors.bankruptcy_history = mockCreditBureauReport && mockCreditBureauReport.bankruptcyHistory ? mockCreditBureauReport.bankruptcyHistory : null;
    // Company identification
    ...
    return {
        ...
        assessment: {
            ...
            reliability: dataSourceReliability ? dataSourceReliability.overallReliability : 0.75,
        ...
        dataSources: dataSourceReliability,
        warnings: companyMatchingWarnings,
        // Additional company details for verification
        companyDetails: {
            name: companyName,
