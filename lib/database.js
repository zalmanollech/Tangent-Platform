...
class Database {
  ...
  // Restore database from provided backup JSON
  // options: { strategy: 'overwrite' | 'merge' }
  restoreFromBackup(backupData, options = {}) {
    ...
    if (DB_TYPE !== 'json') {
      throw new Error('Restore is only supported for JSON database type at this time');
    }

    // Load current data to help with sanitization/merging
    const currentData = this.loadData();

    // Ensure schema on incoming backup
    let candidateData = this.ensureDBSchema(backupData);

    // Preserve sensitive secrets if they were redacted in backup
    if (Array.isArray(candidateData.users)) {
      candidateData.users = candidateData.users.map(userFromBackup => {
        const existing = Array.isArray(currentData.users)
          ? currentData.users.find(u => u.email && userFromBackup.email && u.email.toLowerCase() === userFromBackup.email.toLowerCase())
          : null;

        const passHashRedacted = typeof userFromBackup.passHash === 'string' && userFromBackup.passHash.includes('[REDACTED]');
        const twoFactorSecretRedacted = userFromBackup.twoFactor && typeof userFromBackup.twoFactor.secret === 'string' && userFromBackup.twoFactor.secret.includes('[REDACTED]');

        return {
          ...userFromBackup,
          passHash: passHashRedacted && existing && existing.passHash ? existing.passHash : userFromBackup.passHash,
          twoFactor: userFromBackup.twoFactor
            ? {
                ...userFromBackup.twoFactor,
                secret: twoFactorSecretRedacted && existing && existing.twoFactor && existing.twoFactor.secret
                  ? existing.twoFactor.secret
                  : userFromBackup.twoFactor.secret
              }
            : userFromBackup.twoFactor
        };
      });
    ...
    let restored;
    if (strategy === 'merge') {
      // Shallow merge objects, de-duplicate arrays by id (users by email)
      restored = { ...currentData };
      Object.keys(candidateData).forEach(key => {
        const incoming = candidateData[key];
        ...
        if (Array.isArray(incoming)) {
          if (key === 'users') {
            ...
            incoming.forEach(u => {
              if (u && u.email) byEmail.set(u.email.toLowerCase(), { ...byEmail.get(u.email.toLowerCase()), ...u });
            });
            restored[key] = Array.from(byEmail.values());
          } else {
            const byId = new Map();
            (Array.isArray(existing) ? existing : []).forEach(r => {
              const id = r && r.id ? r.id : JSON.stringify(r);
              byId.set(id, r);
            });
            incoming.forEach(r => {
              const id = r && r.id ? r.id : JSON.stringify(r);
            ...
            restored[key] = Array.from(byId.values());
          }
        } else if (incoming && typeof incoming === 'object') {
          restored[key] = { ...(existing || {}), ...incoming };
        } else {
          restored[key] = incoming;
        }
      });
      restored = this.ensureDBSchema(restored);
    } else {
      // overwrite
      restored = candidateData;
    }
    ...
    return {
      ...
      collections: {
        ...
        trades: Array.isArray(this.data.trades) ? this.data.trades.length : 0,
        complianceChecks: Array.isArray(this.data.complianceChecks) ? this.data.complianceChecks.length : 0,
        documentVerifications: Array.isArray(this.data.documentVerifications) ? this.data.documentVerifications.length : 0
      }
    };
  }
