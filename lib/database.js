...
async function createTables() {
    try {
        ...
        // Contracts table (for future PostgreSQL migration from in-memory storage)
        await client.query(`
            CREATE TABLE IF NOT EXISTS contracts (
                ...
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                -- Risk engine fields (STEP 1: nullable, no logic applied)
                risk_score INTEGER,
                risk_band VARCHAR(50),
                max_financing_percent DECIMAL(5, 2),
                required_deposit_percent DECIMAL(5, 2)
            )
        `);
        
        // Add risk fields to existing contracts table if they don't exist (idempotent migration)
        await client.query(`
            DO $$ 
            BEGIN
                ...
                -- Add risk_band if it doesn't exist
                IF NOT EXISTS (
                    SELECT 1 FROM information_schema.columns 
                ...
                -- Add max_financing_percent if it doesn't exist
                IF NOT EXISTS (
                    SELECT 1 FROM information_schema.columns 
                ...
                -- Add required_deposit_percent if it doesn't exist
                IF NOT EXISTS (
                    SELECT 1 FROM information_schema.columns 
        ...
        `);
        await client.query(`
            CREATE INDEX IF NOT EXISTS idx_contracts_buyer_email ON contracts(buyer_email)
        `);
        await client.query(`
            CREATE INDEX IF NOT EXISTS idx_contracts_supplier_email ON contracts(supplier_email)
        `);
        await client.query(`
...
// Contract operations (for risk engine integration)
// Note: Contracts are primarily stored in-memory (database.contracts Map),
// but we also support PostgreSQL storage for future migration
const contracts = {
    /**
     * Get contract by ID from in-memory store or PostgreSQL
     * @param {string} contractId - Contract ID
     * @param {Map} inMemoryContracts - In-memory contracts Map (from server)
     * @returns {Promise<Object|null>} Contract object or null if not found
     */
    async getById(contractId, inMemoryContracts = null) {
        // First try in-memory store (primary storage)
        if (inMemoryContracts && inMemoryContracts.has(contractId)) {
            return inMemoryContracts.get(contractId);
        }
        ...
        try {
            ...
            if (dbPool) {
                ...
                if (result.rows.length > 0) {
                    ...
                    // Convert database row to contract object format
                    return {
                        id: dbContract.id,
                        contract_id: dbContract.contract_id || dbContract.id,
                        buyerEmail: dbContract.buyer_email,
                        supplierEmail: dbContract.supplier_email,
                        product: dbContract.product || dbContract.product_details,
                        productDetails: dbContract.product_details || dbContract.product,
                        quantity: parseFloat(dbContract.quantity) || 0,
                        unit: dbContract.unit,
                        pricePerUnit: parseFloat(dbContract.price_per_unit) || 0,
                        totalValue: parseFloat(dbContract.total_value) || 0,
                        currency: dbContract.currency || 'USD',
                        depositPercent: parseFloat(dbContract.deposit_percent) || 0,
                        depositAmount: parseFloat(dbContract.deposit_amount) || 0,
                        status: dbContract.status,
                        riskScore: dbContract.risk_score,
                        riskBand: dbContract.risk_band,
                        maxFinancingPercent: dbContract.max_financing_percent ? parseFloat(dbContract.max_financing_percent) : null,
                        requiredDepositPercent: dbContract.required_deposit_percent ? parseFloat(dbContract.required_deposit_percent) : null,
                        ...(dbContract.contract_data || {})
                    };
                }
            }
        } catch (error) {
            console.error('[ERROR] Failed to get contract from PostgreSQL:', error.message);
            // Continue - in-memory is primary
        }
    ...
    /**
     * Update only risk fields for a contract
     * Updates both in-memory store and PostgreSQL (if available)
     * @param {string} contractId - Contract ID
     * @param {Object} riskFields - Risk fields to update
     * @param {Map} inMemoryContracts - In-memory contracts Map (from server)
     * @returns {Promise<void>}
     */
    async updateRiskFields(contractId, riskFields, inMemoryContracts = null) {
        const { riskScore, riskBand, maxFinancingPercent, requiredDepositPercent } = riskFields;
        
        // Update in-memory store (primary storage)
        if (inMemoryContracts && inMemoryContracts.has(contractId)) {
            const contract = inMemoryContracts.get(contractId);
            contract.riskScore = riskScore ?? null;
            contract.riskBand = riskBand ?? null;
            contract.maxFinancingPercent = maxFinancingPercent ?? null;
            contract.requiredDepositPercent = requiredDepositPercent ?? null;
            inMemoryContracts.set(contractId, contract);
        }
        
        // Also update PostgreSQL if available (for future migration)
        try {
            const dbPool = await getPool();
            if (dbPool) {
                await query(
                    `UPDATE contracts 
                     SET risk_score = $1, 
                         risk_band = $2, 
                         max_financing_percent = $3, 
                         required_deposit_percent = $4,
            ...
            console.error('[ERROR] Failed to update contract risk fields in PostgreSQL:', error.message);
