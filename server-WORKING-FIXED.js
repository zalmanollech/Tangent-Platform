        ...
        function getActionButtons(contract, userRole) {
            ...
            if (userRole === 'buyer') {
                ...
                if (contract.status === 'active' && contract.depositPaid && contract.documentsUploaded) {
                    ...
                    buttons += '<button class="btn secondary small" onclick="releasePayment('+JSON.stringify(contract.id)+', '+remainingAmount+')" style="background: #10b981;">Release Payment ($'+remainingAmount.toLocaleString()+')</button>';
                }
                // Show waiting status
                if (contract.status === 'active' && contract.depositPaid && !contract.documentsUploaded) {
                ...
                if (contract.status === 'pending_supplier_confirmation') {
                    buttons += '<button class="btn secondary small" onclick="confirmContract('+JSON.stringify(contract.id)+')">Confirm Contract</button>';
                }
                ...
                buttons += '<button class="btn secondary small" onclick="manageTraderContract('+JSON.stringify(contract.id)+')">Dual Contract</button>';
            }
